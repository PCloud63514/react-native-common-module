...
declare global {
    interface IRestfulService extends EventListener {
        Request(ruleName: string, method: Method, dataBundle?: any):Promise<any>
        RetryRequest(fixRequestConfig: any): Promise<any>
    ...
    async sequentialRequest(rest:IRestfulService, isCahceFlush:boolean=true):Promise<any> {
        ...
        for(const {key, ruleName, method, dataBundle} of this._queue) {
            ...
            if(dataBundle === undefined) {
                response = await rest.Request(ruleName, method, this._responseCacheObj)
                ...
                response = await rest.Request(ruleName, method, dataBundle)
            } else {
                response = await rest.Request(ruleName, method, dataBundle)
            }
...
class RestfulService extends EventListener implements IRestfulService {
     ...
     */
    async Request(ruleName: string, method: Method, dataBundle?: any): Promise<any> {
        let requestConfig = await this.generateRequestConfig(ruleName, method, dataBundle)
        return await this._instance?.request(requestConfig)
    }


    /** 재시도 요청 (config을 변경하지 않는다. 그대로 요청) */
    async RetryRequest(fixRequestConfig: any): Promise<any> {
